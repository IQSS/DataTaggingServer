        ...
        kits.get(requestedInterview.kitId) match {
          ...
          case Some(kit) => {
            for{
              ...
              val userSession = InterviewSession.create(kit, vpmOpt.exists(vpm => vpm.saveStat), vpmOpt.exists(vpm => vpm.notesAllowed)).updatedWithRequestedInterview(requestedInterview)
